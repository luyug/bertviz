...
def head_view(attention, tokens, sentence_b_start = None, prettify_tokens=True):
    ...
    bsz, n_heads, q_len, d_len = attention[0].shape
    pseudo_full = [torch.zeros(bsz, n_heads, q_len+d_len, q_len+d_len) for _ in range(len(attention))]
    cross_attn = format_attention(attention)
    attn = format_attention(pseudo_full)
    attn_data = {
        'all': {
            'attn': attn.tolist(),
            'left_text': tokens,
